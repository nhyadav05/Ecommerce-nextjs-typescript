...
//               <span
//                 aria-hidden="true"
//                 className="absolute inset-0 pointer-events-none -inset-px rounded-md border-2 border-gray-200"
//               >
//                 <svg
...
interface Props {
  productId: string; // Prop to pass product ID dynamically
}
...
const ProductDetails: React.FC<Props> = ({ productId }) => {
  ...
  return (
    <div>
      <h1>{product.name}</h1>
      <img
        src={product.images[0]} // Displaying first image for simplicity
        alt={product.name}
        style={{ maxWidth: "100%", maxHeight: "400px" }}
      />
      <p>Price: ${product.price}</p>
      <p>Launch Date: {new Date(product.launchDate).toLocaleDateString()}</p>
