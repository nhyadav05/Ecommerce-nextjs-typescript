...
const CartPage: React.FC = () => {
  ...
  const handleRemove = async (productId: string) => {
    try {
      ...
      const count = updatedCartItems.reduce((acc, item) => acc + item.quantity, 0);
      cookies.set("cartItems", updatedCartItems);
    } catch (error) {
      console.error("Error removing item from cart:", error);
      setError("Error removing item from cart. Please try again later.");
  ...
  const handleIncreaseQuantity = async (productId: string) => {
    try {
      await incrementCartItemQuantity(productId, userId);
      const response = await fetchCartItems(userId); // Fetch updated cart items from server
      setCartItems(response.data.carts);
      setTotalPrice(parseFloat(response.data.totalPrice)); // Update totalPrice from server response
    } catch (error) {
      console.error("Error increasing quantity:", error);
      setError("Error increasing quantity. Please try again later.");
    }
  ...
  const handleDecreaseQuantity = async (productId: string) => {
    try {
      ...
      await decrementCartItemQuantity(productId, userId);
      const updatedCartItems = cartItems.map((item) =>
